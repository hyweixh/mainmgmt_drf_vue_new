<script setup name="userinfo_list">
...
const onEditDeviceInfo = (index) => {
    ...
    // console.log("device=====",device,device.id)
    router.push({ name: 'devicemgmt_edit', params: { id: device.id } });
    // 使用 query 传递 page 和 page_size  
    //  router.push({  
    //     name: 'devicemgmt_edit',  
    //     params: { id: device.id }, // 如果 id 是路由的一部分，使用 params  
    //     query: { page: 2, pageSize: page_size } // 使用 query 传递其他参数  
    // });  
};
...
const onShowCopyPwdDialog = (index) => {
    ...
    passpwds.pwd1 = device.pwd1
    passpwds.pwd2 = device.pwd2
    passpwds.pwd3 = device.pwd3
    passpwds.pwd4 = device.pwd4
    // selectedAccount = 'user1'; // 打开对话框时重置selectedAccount
...
const onCopyPwd = async (user) => {
    ...
    switch (user) {
        ...
        case 'user4':
            ...
            break;
        default:
            ElMessage.error('请选择一个有效的用户.');
            return; // 提前退出函数  
    }
    // console.log("密码是：",pwd)
    try {  
        const decrypt_passwd = await devicemgmtHttp.getDecodePwd(pwd);  
        // console.log("onCopypwd----",decrypt_passwd)
        if (decrypt_passwd && decrypt_passwd.decoded_password) {  
            navigator.clipboard.writeText(decrypt_passwd.decoded_password);  
            ElMessage.success('密码拷贝到剪切板成功!');  
            copyPwdDialogVisible.value = false;  
        } else {  
            ElMessage.error('后端返回的数据不包含解密后的密码.');  
        }  
    } catch (error) {  
        ...
        <el-card>
            <el-form :inline="true" class="my-form-inline">
                <el-form-item label="按子网">
                    <el-select v-model="filterForm.subnetwork_id" placeholder="请选择子网类型">
                        <el-option v-for="subnettype in subnettypes" :key="subnettype.id"
                            :label="subnettype.subnettypename" :value="subnettype.id" />
                    </el-select>
                </el-form-item>
                <el-form-item label="按安装位置">
                    <el-input v-model="filterForm.position" />
                </el-form-item>
                <el-form-item label="按设备类型">
                    <el-select v-model="filterForm.devicetype_id" placeholder="请选择设备类型">
                ...
                <el-form-item>
                    ...
                    <el-button type="primary" icon="Plus" @click="navigateToAddDevice">新增设备</el-button>
                </el-form-item>            

                <el-form-item>
                    <el-upload 
                        :action="BASE_URL + '/devicemgmt/upload/'" 
...
.vertical-divider {
    ...
    width: 2px;               /* 竖线的宽度 */
    height: 30px;             /* 竖线的高度，可以根据需要调整 */
    margin: 0 10px;           /* 左右间距，可以根据需要调整 */
    background-color: #cccccc; /* 竖线的颜色，可以根据需要调整 */
    vertical-align: middle;   /* 使竖线垂直居中 */
}
</style>