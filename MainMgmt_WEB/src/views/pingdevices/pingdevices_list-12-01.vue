<template>
  <div>
    <!-- 操作区域：显示当前检查人员 + 执行按钮 -->
    <el-card :gutter="20" style="margin-bottom: 20px">
      <el-col :span="6">
        <el-button 
...
const pagedResultList = computed(() => {
  ...
  return resultList.value.slice(start, end)
})
// ==================== 计算属性：当前登录用户 ====================
...
async function startBatchPing() {
  ...
  try {
    ...
    console.log("获取的设备信息：",deviceData)
